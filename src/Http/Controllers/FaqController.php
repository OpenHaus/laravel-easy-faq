...
class FaqController extends Controller
{
    ...
    public function index()
    {
        ...
        $aFaq = DB::select(DB::raw('select id, faq_category_id, question, likes from ( select id, faq_category_id, question, likes, (@rn:=if(@prev = category_id, @rn +1, 1)) as rownumb, @prev:= faq_category_id from ( select id, faq_category_id, question, likes from faq order by faq_category_id ,  likes desc, id ) as sortedlist JOIN (select @prev:=NULL, @rn :=0) as vars ) as groupedlist where rownumb<=3 order by faq_category_id, likes desc, id'));
    ...
    public function show($id)
    {
        ...
        $aFaq = Faq::where('faq_category_id', '=', $faq->faq_category_id)->where('id', '!=', $faq->id)->orderBy('likes', 'desc')->get();
        $cFaq = $aFaq->count()+1;
